!!!!!!!!
!
!  modified version of betts_miller.f90
!   as provided by Dargan Frierson
!  implementing the Simplified Betts Miller convection scheme
!
!  For CLIMLAB we make the following modifications:
!  - no module, just a collection of subroutines to be wrapped by f2py
!  - no i/o from the Fortran layer
!  - all parameters and namelist values passed as explicit inputs
!  - get rid of subroutine betts_miller_init (since we do not read namelists)
!  - explicit array dimensions for all input and output variables
!  - pass grid dimensions are input variables
!  - include an explicit subroutine escomp for calculating saturation vapor pressure
!  - remove optional input arguments mask and conv (not implemented)
!  - remove input field coldT and output field snow. These are not implemente in Dargan's code
!   (all precip is rain)
!
!   Brian Rose
!   July 2019
...
subroutine escomp(T,es)
  ...
  !   Output: saturation vapor pressure in Pascals
  ...
  real:: Tcel
  Tcel = T - 273.15
  es = 611.2 * exp(17.67*Tcel/(Tcel+243.5))
